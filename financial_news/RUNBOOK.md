...
- source/topic note maintenance so those MOCs accumulate useful backlinks
...
- `--attachment-mode preserve` keeps the original file format and metadata-preserving copy behavior
- `--attachment-mode optimize` can resize/recompress static images into `webp`, `jpeg`, or `png`
- optimization is conservative: if the transformed file is not smaller, the importer keeps the preserved original instead
- animated or unreadable images fall back to the preserved original
- exact destination matches are still reused instead of recopied

## Safe pruning stance

There is no autonomous background prune mode, but there is an explicit cleanup flag.

Safe operator stance:

- do not assume `--reset-state` cleans anything except the cursor
- use `--prune-orphan-attachments` only when you intentionally want to remove copied files no longer referenced by notes
- pruning touches only files under `attachments/`; it does not bulk-delete day-note folders
- verify the files are truly orphaned in the real vault before running prune against production output

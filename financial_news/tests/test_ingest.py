...
from financial_news.config import AttachmentConfig
...
def test_main_passes_attachment_config_to_append_summary(tmp_path: Path, monkeypatch) -> None:
    output_root = tmp_path / "vault"
    state_path = tmp_path / "state" / "ingest_state.json"
    seen = {}
    ...
    def fake_append_summary(output_root_arg, record, attachment_config=None):
        seen["attachment_config"] = attachment_config
    ...
    assert result == 0
    assert isinstance(seen["attachment_config"], AttachmentConfig)
    assert seen["attachment_config"].mode == "preserve"
    assert seen["attachment_config"].path_remaps[0].source == Path("/Users/attila/d-ai-trader")
    assert seen["attachment_config"].path_remaps[0].destination == Path("/Volumes/adobi/d-ai-trader")
...
def test_main_checkpoints_each_successful_record_before_failure(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_append_summary(output_root: Path, record: SummaryRecord, attachment_config=None) -> Path:
